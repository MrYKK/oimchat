 ...
 * 
 * @author: XiaHui
 * @date 2018-01-18 22:09:09
 */
public class GroupChatHistoryViewImpl extends AbstractView implements GroupChatHistoryView {

	ChatHistoryFrame frame;// = new ChatHistoryFrame();
	String ownerUserId;
	String groupId;

	public GroupChatHistoryViewImpl(AppContext appContext) {
		super(appContext);
	...
	public void setList(List<GroupChatHistoryData> contents, PageData page) {
		...
		if (null != contents) {
			...
			for (int i = size - 1; i >= 0; i--) {
				GroupChatHistoryData chd = contents.get(i);

				UserData userData = chd.getUserData();
				String chatUserId = userData.getId();
				...
				String head = him.getUserHeadPath(userData.getId());
				...
				String name = isOwn ? "我" : AppCommonUtil.getDefaultShowName(userData);
		...
		if (null != contents) {
			for (GroupChatHistoryData chd : contents) {
				Content content = chd.getContent();
